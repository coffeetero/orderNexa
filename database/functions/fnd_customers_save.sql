...
BEGIN
  ...
  IF v_action = 'create' THEN
    ...
    VALUES (
      ...
      nullif(p_payload ->> 'billing_period',  ''),
      CASE WHEN p_payload ? 'credit_limit' AND p_payload ->> 'credit_limit' <> '' THEN (p_payload ->> 'credit_limit')::numeric ELSE NULL END,
  ...
  ELSIF v_action = 'update' THEN
    ...
    SET
      ...
      billing_type   = CASE WHEN p_payload ? 'billing_type'   THEN nullif(p_payload ->> 'billing_type',  '') ELSE cus.billing_type   END,
      billing_period = CASE WHEN p_payload ? 'billing_period' THEN nullif(p_payload ->> 'billing_period', '') ELSE cus.billing_period END,
      credit_limit   = CASE WHEN p_payload ? 'credit_limit'   THEN CASE WHEN p_payload ->> 'credit_limit' = '' THEN NULL ELSE (p_payload ->> 'credit_limit')::numeric END ELSE cus.credit_limit END,
