-- ============================================================
-- FND_CUSTOMERS_GET_V2 — RLS-respecting version (no SECURITY DEFINER)
-- Run after: fnd_customers.sql and fnd_customers_policies.sql
-- ============================================================

DROP FUNCTION IF EXISTS bps.fnd_customers_get_v2(bigint, bigint, boolean, boolean);

CREATE OR REPLACE FUNCTION bps.fnd_customers_get_v2(
    p_tenant_id bigint,
...
BEGIN
    /*
     * BPS Foundation: Unified customer fetch (RLS-respecting version).
     *
     * Identical logic to fnd_customers_get but WITHOUT SECURITY DEFINER,
     * so RLS policies on fnd_customers are enforced.
     *
     *   p_hierarchy = false
